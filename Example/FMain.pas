...
begin
{Запрашиваем разрешение на использование камеры во время работы программы}
{$IFDEF ANDROID}
       ...
       begin
          ...
          begin
            ...
            0:
              begin
                  CameraComponent1.Active := false;
                  // Выбираем фронтальную или заднюю камеру устройства
                  ...
                  // Настраиваем качество изображения с камеры
          ...
          begin
            ...
            0:
                {$IFDEF ANDROID}
                    // В случае отмены разрешения выводим тост
                    TJToast.JavaClass.makeText(TAndroidHelper.Context, StrToJCharSequence('Использование камеры запрещено!'), TJToast.JavaClass.LENGTH_LONG).show;
                {$ENDIF}
            end;
          end;
       end;
...
begin
// переключаем видимость компонента
  Layout1.Opacity:=0.0;
...
// Выбор нужного качества изображения с камеры
